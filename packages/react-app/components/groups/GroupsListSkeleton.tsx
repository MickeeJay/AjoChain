export function GroupsListSkeleton() {
  return (
    <section className="grid animate-pulse gap-4 md:grid-cols-2">
      {[1, 2, 3, 4].map((item) => (
        <article key={item} className="rounded-[1.5rem] border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950/90">
          ...
          <div className="mt-4 h-10 w-36 rounded-full bg-slate-200 dark:bg-slate-800" />
        </article>
      ))}
    </section>
  );
